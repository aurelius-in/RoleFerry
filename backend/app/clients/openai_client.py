        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "Positions/work experience — CRITICAL RULES:\n"
                    "- Extract EVERY role in work history (do not stop at the first one, do not skip any).\n"
                    "- title: the actual job title ONLY. Maximum 6 words. Never include company name, location, description phrases, or personal summaries like 'passionate about scaling evidence-based...' in the title field. If you can't identify a clean title, use the first bolded or capitalized line under each experience section.\n"
                    "- company: the EMPLOYER NAME only. This is NEVER the candidate's own name. If the resume says 'Jane Smith' at the top, that is the candidate, not a company. Look for org/institution names below each date range.\n"
                    ...
                    "- COMMON MISTAKES TO AVOID: (1) Setting title to a sentence or bio fragment. (2) Setting company to the candidate's own name. (3) Missing positions because the resume has unusual formatting. (4) Merging multiple roles into one.\n\n"
                    "Key metrics:\n"
                    ...
                    "- Extract ALL education entries (degrees, MBAs, PhDs, certifications, bootcamps).\n"
                    "- Do NOT stop at the first one. A candidate may have BA + MA + PhD — capture all three.\n"
                    "- school: the institution name only.\n"
                    "- degree: the degree type (BA, MBA, PhD, MS, BS, Certificate, etc.).\n"
                    "- field: the subject/concentration.\n"
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "- If the job post has no explicit qualifications/tools, infer 6–10 capability-style skills from responsibilities (e.g., 'Technical scoping', '0→1 product development', 'Client communication').\n"
                    ...
                    "- If the post includes 'Who this is for / not for', convert those into requirements and exclusions.\n"
                    "- If the post includes location restrictions (e.g., 'You must be located in...'), include that.\n"
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "- jd_snippet MUST be a concrete responsibility / problem-to-solve. Avoid generic slogans, marketing copy, or perks.\n"
                    "- jd_snippet MUST NOT be a section header (no trailing ':').\n"
    ...
    def draft_compose_email(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "- custom: treat `custom_tone` as the style guide and follow it exactly.\n"
                    "  - Do what it asks (voice, length, formality, sentence style, punctuation).\n"
                    "  - Avoid what it forbids.\n"
                    "  - Do NOT mention the instruction; just write in that tone.\n\n"
                    ...
                    "- If `bio_page_url` exists, include ONE short line like \"Bio: {{bio_page_url}}\".\n"
                    "- If `personalization_angle` is non-empty, include ONE sentence that references it (keep it <= 22 words).\n"
                    "\nTemplate rules:\n"
                    "- Preserve placeholders exactly (e.g., {{first_name}}, {{job_title}}, {{company_name}}, {{contact_title}}, {{personalization_angle}}, {{painpoint_1}}, {{solution_1}}, {{metric_1}}, {{offer_snippet}}, {{work_link}}, {{bio_page_url}}).\n"
                    "- Do NOT replace placeholders with actual names/companies.\n"
                    "- Keep the body 90–140 words when possible.\n"
        ...
        stub = {
            ...
            "body": (
                ...
                "{{personalization_angle}}\n\n" if str((context or {}).get("personalization_angle") or "").strip() else ""
                "- One idea I’d bring: {{offer_snippet}}\n"
                "- Relevant proof: {{solution_1}} ({{metric_1}})\n\n"
                "Open to a quick 10-15 minute chat?\n\n"
                "Bio: {{bio_page_url}}\n\n" if str((context or {}).get("bio_page_url") or "").strip() else ""
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "- Do NOT duplicate 'Best,' or any closing. There should be exactly one sign-off.\n"
